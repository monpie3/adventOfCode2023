...
direction_dict = {"0": (0, 1), "2": (0, -1), "3": (-1, 0), "1": (1, 0)}
...
def translate_instruction(instruction):
    instruction = instruction[2:-1]  # omit (# and )
    direction = instruction[-1]
    step_num = instruction[:-1]
    return direction, int(step_num, 16)
...
def lagoon_capacity(filename) -> float:
    ...
    with open(filename) as f:
        for line in f:
            ...
            direction, step_num = translate_instruction(instr)
            perimeter += step_num
            current_pos = move_to_next_edge(current_pos, direction, step_num)
